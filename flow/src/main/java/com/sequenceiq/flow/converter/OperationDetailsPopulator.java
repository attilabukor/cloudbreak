...
public class OperationDetailsPopulator {
    ...
    public OperationView createOperationView(OperationFlowsView operationFlowsView,
        ...
        response.setExpectedNumberOfFlows(typeOrderList.size());
        ...
        populateOperationDetails(response, responseListToProcess);
        return response;
    }

    private void populateOperationDetails(OperationView source,
            List<Optional<FlowProgressResponse>> operations) {
        int preProgress = 0;
        ...
        if (CollectionUtils.isNotEmpty(source.getOperations())) {
            int fullProgress = overallProgress / source.getExpectedNumberOfFlows();
            ...
            if (fullProgress == MAX_PROGRESS) {
                ...
                source.setProgressStatus(OperationProgressStatus.PENDING);
            }
        } else {
    ...
    private OperationProgressStatus calculateProgress(List<FlowProgressResponse> provisionFlows) {
        ...
        if (hasFailedStatus) {
            ...
            progressStatus = OperationProgressStatus.PENDING;
        }
