...
public class AwsNativeMetadataCollector implements MetadataCollector {
    ...
    private List<CloudVmMetaDataStatus> collectInstances(List<CloudInstance> vms, List<CloudResource> resources, AmazonEc2Client ec2Client) {
        ...
        Map<String, CloudResource> resourcesByPrivateId = resources.stream()
                .filter(resource -> ResourceType.AWS_INSTANCE.equals(resource.getType()))
                .collect(Collectors.toMap(CloudResource::getReference, Function.identity()));
        ...
        for (Map.Entry<Integer, List<String>> instanceIdBatchEntry : instanceIdBatches.entrySet()) {
            ...
            try {
                List<CloudVmMetaDataStatus> metaDataStatuses = describeInstances(vms, instanceIdBatch, resourcesByInstanceId, resourcesByPrivateId, ec2Client);
    ...
    private List<CloudVmMetaDataStatus> handleNotFoundInstancesAndDescribeOthers(List<CloudInstance> vms, AmazonEc2Client ec2Client,
            ...
            Map<String, CloudResource> resourcesByPrivateId) {
        ...
        if (!existingInstanceIds.isEmpty()) {
            ...
            result.addAll(describeInstances(vms, existingInstanceIds, resourcesByInstanceId, resourcesByPrivateId, ec2Client));
        }
        return result;
    }

    private List<CloudVmMetaDataStatus> describeInstances(List<CloudInstance> vms, List<String> knownInstanceIdList,
            Map<String, CloudResource> resourcesByInstanceId, Map<String, CloudResource> resourcesByPrivateId, AmazonEc2Client ec2Client) {
        ...
        for (Instance instance : instances) {
            ...
            CloudResource cloudResource = instanceId != null ? resourcesByInstanceId.get(instanceId) : resourcesByPrivateId.get(String.valueOf(privateId));
            CloudInstance updatedInstance = new CloudInstance(cloudResource.getReference(), cloudInstance.getTemplate(), cloudInstance.getAuthentication());
