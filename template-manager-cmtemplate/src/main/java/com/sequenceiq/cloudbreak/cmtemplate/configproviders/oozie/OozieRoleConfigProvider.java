        ...
        switch (roleType) {
            case OozieRoles.OOZIE_SERVER:
                ...
                if (isOozieHA(source) && CloudPlatform.GCP == source.getCloudPlatform()) {
                    // There is no load-balancer for GCP yet. If the first Oozie server is down, the second Oozie server can process actions.
                    // But without the load-balancer the callbacks from Yarn will fail. By default the action check delay is 10 mins.
                    // Reducing this to 2 mins.
                    config.add(config(OOZIE_CONFIG_SAFETY_VALVE,
                        "<property><name>oozie.service.ActionCheckerService.action.check.delay</name><value>120</value></property>"));
                }
                return config;
            default:
                return List.of();
