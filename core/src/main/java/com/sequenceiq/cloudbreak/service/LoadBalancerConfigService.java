...
public class LoadBalancerConfigService {
    ...
    public Optional<LoadBalancer> getLoadBalancerWithEndpointDefined(Long stackId, LoadBalancerType loadBalancerType) {
        return loadBalancerPersistenceService.findByStackId(stackId).stream()
            .filter(lb -> StringUtils.isNotBlank(lb.getDns()) ||
                StringUtils.isNotBlank(lb.getIp()) || StringUtils.isNotBlank(lb.getFqdn()))
    ...
    public String getInternalLoadBalancerEndpoint(Long stackId) {
        return getLoadBalancerWithEndpointDefined(stackId, LoadBalancerType.PRIVATE).map(this::getBestAddressable).orElse(null);
    }
